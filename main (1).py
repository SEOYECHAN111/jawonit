...
# ============================================================
# Render root route fix (v51.12.2)
# - Render에서 https://...onrender.com/ 접속 시 {"detail":"Not Found"}가 뜨는 문제 방지
# - static/index.html을 명시적으로 홈으로 반환
# - /index.html, /admin-dashboard.html 등 루트 HTML 파일도 안전하게 반환
# - 이 라우트들은 반드시 app.mount('/', StaticFiles(...))보다 위에 있어야 함
# ============================================================
def _safe_static_html_response(filename: str):
    safe = str(filename or '').replace('\\', '').replace('/', '').strip()
    ...
    return JSONResponse(
        {
            ...
            'hint': 'GitHub 저장소 루트에 static/index.html과 관련 HTML 파일이 올라갔는지 확인하세요.'
        },
        status_code=404,
    )

@app.get('/', include_in_schema=False)
def render_home_route_fix():
    ...
    return JSONResponse(
        {
            ...
            'static_dir': str(STATIC),
            'hint': 'Render 배포 저장소의 루트 구조가 main.py / requirements.txt / static/index.html 형태인지 확인하세요.'
        },
        status_code=404,
    )

@app.get('/index.html', include_in_schema=False)
def render_index_route_fix():
    return _safe_static_html_response('index.html')

@app.get('/{page_name}.html', include_in_schema=False)
def render_root_html_route_fix(page_name: str):
    return _safe_static_html_response(f'{page_name}.html')
